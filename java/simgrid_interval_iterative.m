function [ps, X, Y] = simgrid_interval_iterative(depth, ps,t,t_next,x0,y0,opt)
...
while t < t_next
    ...
    if ~all(is_first_subgraph)
        ...
        [ps,~,X,Y] = superset_odeout(ps,ps1,ps2,t_out1,t_out2,X1,Y1,X2,Y2,opt);
%         t_out = [t_out t_step]; X_out = [X_out X_step]; Y_out = [Y_out Y_step];
        break;
    elseif ps.blackout || (n_macs == 0 || n_shunts == 0 || (ps.shunt(:,C.sh.P)'*ps.shunt(:,C.sh.factor) == 0) || size(ps.bus(:,1),1) == 1)
        if opt.verbose
            disp( '!!! Blackout or trivial island' );
        end
        ...
        try
            ...
            switch ex.identifier
                case 'Cosmic:NotConverged'
                    if opt.verbose
                        disp( '!!! Cosmic:NotConverged in solve_dae' );
                    end
                    ps = removeTempRef( ps, temp_ref, ref );
                    [ps, ~, X, Y] = onBlackout( ps, t, x0, y0 );
%                     t_out = [t_out t_step]; X_out = [X_out X_step]; Y_out = [Y_out Y_step];
...
%         t_out = [t_out t_step]; X_out = [X_out X_step]; Y_out = [Y_out Y_step];
    ...
    %         [ps,t_out_down,X_down,Y_down] = ...
    %             simgrid_interval(depth+1, ps,t_down,t_next,x_end,y_end,opt);
...
%         disp( '...Overwriting ps.branch' );
        ps.branch(:, C.br.var_idx) = 0;
    end
    if ~isempty( ps.shunt )
%         disp( '...Overwriting ps.shunt' );
